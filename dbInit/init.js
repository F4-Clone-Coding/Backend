...
async function createStores() {
  ...
  for (let i = 0; i + 100 <= 20000; i += 100) {
    const stores = storeData.slice(i, i + 100).map((store) => {
      ...
      const viewRecent =
        (viewTotal > randomViewCount() * 0.1) | 0
          ? (randomViewCount() * 0.1) | 0
...
async function createMenus() {
  ...
  menuPrice.map((v, i) => {
    ...
    else if (menu.match(/라떼|아메리카노|아인슈|콜드/))
      menuCategoryList.push(18);
    ...
    else if (menu.match(/탕수육|살|떡볶이|짜장면|게장|순대|조림|고기|롤/))
