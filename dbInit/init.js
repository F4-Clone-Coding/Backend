const storeData = require('../utils/listing/data');
const apvDate = require('../utils/listing/apvDate');
...
async function createStores() {
  ...
  for (let i = 0; i + 100 <= 20000; i += 100) {
    ...
    const stores = storeData.slice(i, i + 100).map((store) => {
      ...
      const viewRecent = (viewTotal > randomViewCount() * 0.1) | 0 ? (randomViewCount() * 0.1) | 0 : (viewTotal * Math.random()) | 0;
...
async function createMenus() {
  ...
  let newData = stores.map((v) => {
    ...
    return {
      name: v['REST_NM'].includes('인생죽') ? v['REST_NM'].replaceAll(' ', '').replaceAll('(', '').replaceAll(')', '') : v['REST_NM'],
